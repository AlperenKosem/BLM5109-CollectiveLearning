...
df_train=pd.read_csv("hw2_a/turkish_movie_sentiment_dataset/turkish_movie_sentiment_dataset.csv")
...
for i in range(num_bagging_iterations):
    X_sampled, y_sampled, mask_sampled = resample(input_ids, labels, attention_masks, random_state=(i+2)*33, n_samples= 12000, replace=True)
...
f = open("hw2b_final_report_for_small_bert_movie_sentiment.txt", "a")
...
for X_sampled, y_sampled, attention_masks in zip(bagged_train, bagged_label, bagged_masks ):
    ...
    if(bag_counter == 0 ) :
        ...
        result = np.apply_along_axis(most_common, axis=0, arr=all_predictions)
