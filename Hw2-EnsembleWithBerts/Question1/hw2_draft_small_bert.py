...
df_train=pd.read_csv("imdb_turkish/train.csv",index_col=[0],encoding="windows-1252")
...
for i in range(num_bagging_iterations):
    X_sampled, y_sampled = resample(comments, labels, random_state=(i+1)*33, replace=True) 
...
for X_sampled, y_sampled in zip(bagged_train, bagged_label):
    ...
    indices=tokenizer.batch_encode_plus(comments,max_length=128,add_special_tokens=True, return_attention_mask=True,padding=True, truncation=True)
    ...
    config = AutoConfig.from_pretrained(
            "ytu-ce-cosmos/turkish-small-bert-uncased",num_labels=2)
    ...
    pickle.dump(model, open(f'small_bert_model_from_bag{bag_counter}_bs{batch_size}_e{epochs}_lr{lr}.pkl', 'wb'))
    ...
    plt.tight_layout()
    plt.savefig(f'small_bert_model_from_bag{bag_counter} Epochs={epochs},LR={lr},bs{batch_size}.png')
