...
model_bag3 = pickle.load(open('model_from_bag3_bs32_e4_lr2e-05.pkl', 'rb'))
model_bag4 = pickle.load(open('model_from_bag4_bs16_e3_lr1e-05.pkl', 'rb'))


df_test=pd.read_csv("hw2_a/turkish_movie_sentiment_dataset/turkish_movie_sentiment_dataset.csv")

comments = lambda x : x[23:-24]

df_test["comment"] = df_test["comment"].apply(comments)
df_test["comment"].head()

floatize = lambda x : float(x[0:-2])

df_test["point"] = df_test["point"].apply(floatize)
df_test["point"].value_counts()

df_test.drop(df_test[df_test["point"] == 3].index, inplace = True)
df_test["point"] = df_test["point"].replace(1, 0)
df_test["point"] = df_test["point"].replace(2, 0)
df_test["point"] = df_test["point"].replace(4, 1)
df_test["point"] = df_test["point"].replace(5, 1)
df_test["point"].value_counts()
...
labels = df_test.point.values

X_sampled, labels = resample(comments, labels,n_samples= 4000, replace=False)
...
f = open("report_for_small_bert_for_movie_sentiment.txt", "a")
