...
mi_model_weights_path = 'wi_weights_common.pth'

model_mi = SimpleCNN()  # Varsayılan bir model oluşturulur
model_mi.load_state_dict(torch.load(f'weights/{mi_model_weights_path}'))


weights_m1 = model_m1.state_dict()
weights_m2 = model_m2.state_dict()
weights_mi = model_mi.state_dict()


weights_new = model_m1.state_dict().copy()
# Öncelikle, alfa değerlerini içeren bir aralık belirleyelim
alfas = np.linspace(1.1, 2, 10)  # 0 ile 1 arasında 11 adet alfa değeri
betas = np.linspace(1.1,2,10)
...
for beta in betas:
    ...
    for alfa in alfas:
        ...
        # Ağırlıkları beta değeri ile birleştirme
        for key in weights_m1.keys():
            if key in weights_m2:
                weights_new[key] = beta * weights_new[key] + (1 - beta) * weights_mi[key]
        ...
        print(f'Beta: {beta}, Alfa: {alfa}, Accuracy: {accuracy}')

        if accuracy > best_accuracy:
            print(f'BESSSTT::  Beta: {beta}, Alfa: {alfa}, Accuracy: {accuracy}')
            best_accuracy = report['accuracy']
            best_beta = beta
            best_alfa = alfa

# Aldığımız accuracy değerlerini görselleştirelim
plt.plot(range(len(alfas)*len(betas)), accuracy_values, marker='o')
plt.xlabel('Alfa*Beta Değeri')
plt.ylabel('Accuracy')
plt.title('Alfa -Beta - Accuracy')
plt.grid(True)
plt.savefig(f'reports/w1_w2_wi_searching_accuracies_triangle_ext.png')

plt.show()

print(f'En yüksek accuracy değeri: {best_accuracy} (Alfa = {best_alfa}) (Beta = {best_beta})')

# Ağırlıkları alfa değeri ile birleştirme
for key in weights_m1.keys():
    if key in weights_m2:
        weights_new[key] = best_beta * (best_alfa * weights_m1[key] + (1 - best_alfa) * weights_m2[key]) + ((1 - best_beta) * weights_mi[key])
...
# Create a grid for alfa and beta values
alfa_grid, beta_grid = np.meshgrid(alfas, betas)

# Reshape the accuracy values to match the grid shape
accuracy_grid = np.array(accuracy_values).reshape(len(betas), len(alfas))
...
# Plot the surface
surf = ax.plot_surface(alfa_grid, beta_grid, accuracy_grid, cmap='viridis')
...
# Set title
plt.title('Alfa - Beta - Accuracy Surface Plot')
